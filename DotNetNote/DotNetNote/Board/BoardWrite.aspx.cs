...
namespace DotNetNote.Board
{
    public partial class BoardWrite : System.Web.UI.Page
    {
        ...
        private string _BaseDir = ""; // GitReponsitory\StudyASPNET21\... Files
        private string _FileName = ""; // 파일명
        private int _FileSize = 0; // 파일사이즈
        protected void Page_Load(object sender, EventArgs e)
        {
            _Id = Request["Id"]; // GET, POST 다 받음
        ...
        protected void btnWrite_Click(object sender, EventArgs e)
        {
            ...
            {
                lblError.Text = "보안코드가 틀립니다. 다시입력하세요";
            }
        }
        ...
        {
            _BaseDir = Server.MapPath("../Files"); // D:\Sources\Gitrepository\StudyAspNet21\DotNetNote\DotNetNote\Files
            _FileName = "";
            _FileSize = 0;
            if (txtFileName.PostedFile != null)
            {
                ...
                {
                    if (FormType == BoardWriteFormType.Modify) // 수정일 경우
                    {
                    ...
                    {
                        // 핵심파일저장 프로세스
                        // 폴더에 이미 test.txt가 있으면 test(1).txt로 변경해주는 역할
        ...
        {
            if (Page.User.Identity.IsAuthenticated) // 이미 로그인한 상태이면
            {
                return true;
            }
            else
